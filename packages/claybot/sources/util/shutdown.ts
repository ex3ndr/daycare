...
let resolveShutdown:
  | ((signal: NodeJS.Signals | "fatal") => void)
  | null = null;
...
export function requestShutdown(
  signal: NodeJS.Signals | "fatal" = "SIGTERM"
): void {
  if (shutdownRequestedSignal) {
    return;
  }
...
async function triggerShutdown(
  ...
  const forceExit = setTimeout(() => {
    logger.warn(`Shutdown: forcing exit after ${FORCE_EXIT_MS}ms`);
    process.exit(1);
  }, FORCE_EXIT_MS);
  forceExit.unref();
  ...
  const totalHandlers = Array.from(snapshot.values()).reduce(
    (total, handlers) => total + handlers.length,
    0
  );

  logger.info(
    `Shutdown: running ${totalHandlers} handler${totalHandlers === 1 ? "" : "s"}`
  );

  const tasks: Promise<unknown>[] = [];
  for (const [name, handlers] of snapshot) {
    if (handlers.length > 0) {
      logger.info(`Shutdown: starting ${handlers.length} handler(s) for ${name}`);
    }
      ...
      tasks.push(
        Promise.resolve()
          ...
          .catch((error) => {
            logger.warn(
              { error },
              `Shutdown: handler ${name}[${index + 1}] failed`
            );
          })
      );
    });
  ...
  if (totalHandlers > 0) {
    logger.info(
      `Shutdown: waiting for ${totalHandlers} handler${
        totalHandlers === 1 ? "" : "s"
      }`
    );
  } else {
    logger.info("Shutdown: no handlers registered");
  }
  ...
  if (tasks.length > 0) {
    ...
    logger.info(
      `Shutdown: completed ${totalHandlers} handler${
        totalHandlers === 1 ? "" : "s"
      } in ${elapsedMs}ms`
    );
  }
