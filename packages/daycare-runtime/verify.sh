...
for ((i=max-1; i>=0; i--)); do
  DAYCARE_ENV_PYTHON_VERSION=${PYTHON[i]:-${PYTHON[0]}} \
  DAYCARE_ENV_NODE_VERSION=${NODE[i]:-${NODE[0]}} \
  DAYCARE_ENV_RUST_VERSION=${RUST[i]:-${RUST[0]}} \
  DAYCARE_ENV_GO_VERSION=${GO[i]:-${GO[0]}} \
  DAYCARE_ENV_SWIFT_VERSION=${SWIFT[i]:-${SWIFT[0]}} \
  DAYCARE_ENV_RUBY_VERSION=${RUBY[i]:-${RUBY[0]}} \
  DAYCARE_ENV_PHP_VERSION=${PHP[i]:-${PHP[0]}} \
  DAYCARE_ENV_JAVA_VERSION=${JAVA[i]:-${JAVA[0]}} \
  bash -lc '
    printf "\n\nTesting setup_daycare with versions:\n"
    env | grep "^DAYCARE_ENV_" | sort
    printf "\n"
