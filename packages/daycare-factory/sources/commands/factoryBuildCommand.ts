 ...
 */
export async function factoryBuildCommand(taskDirectory: string, options: FactoryBuildCliOptions): Promise<void> {
    const paths = factoryBuildPathsResolve(taskDirectory, options.environment, options.config, options.out);
    ...
    const effectiveConfig: FactoryConfigResolved = {
        ...
        removeContainerOnExit: options.keepContainer ? false : config.removeContainerOnExit,
        removeExistingContainer: options.removeExisting ? config.removeExistingContainer : false
    };
