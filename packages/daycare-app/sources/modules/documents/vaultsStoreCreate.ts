...
import { vaultHistoryFetch } from "./vaultHistoryFetch";
import { vaultsFetch } from "./vaultsFetch";
import type { VaultItem, VaultTreeNode, VaultVersion } from "./vaultsTypes";
 ...
 */
export function vaultsStoreCreate() {
    return create<VaultsStore>((set, get) => ({
        items: [],
        tree: [],
        selectedId: null,
        ...
        fetch: async (baseUrl, token, workspaceId) => {
            ...
            try {
                const items = await vaultsFetch(baseUrl, token, workspaceId);
        ...
        fetchHistory: async (baseUrl, token, workspaceId, id) => {
            ...
            try {
                const history = await vaultHistoryFetch(baseUrl, token, workspaceId, id);
        ...
        createDocument: async (baseUrl, token, workspaceId, input) => {
            ...
            try {
                await vaultCreate(baseUrl, token, workspaceId, input);
        ...
        updateDocument: async (baseUrl, token, workspaceId, id, input) => {
            set({ saving: true, error: null });
            try {
                const updated = await vaultUpdate(baseUrl, token, workspaceId, id, input);
                const { items } = get();
                const nextItems = items.map((d) => (d.id === id ? updated : d));
                const tree = vaultTreeBuild(nextItems);
        ...
        deleteDocument: async (baseUrl, token, workspaceId, id) => {
            set({ saving: true, error: null });
            try {
                await vaultDelete(baseUrl, token, workspaceId, id);
        ...
        move: async (baseUrl, token, workspaceId, id, newParentId) => {
            set({ saving: true, error: null });
            try {
                await vaultUpdate(baseUrl, token, workspaceId, id, { parentId: newParentId });
                await get().fetch(baseUrl, token, workspaceId);
                set({ saving: false });
            } catch (err) {
