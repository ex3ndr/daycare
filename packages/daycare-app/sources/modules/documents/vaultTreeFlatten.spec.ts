...
import { vaultTreeFlatten } from "./vaultTreeFlatten";
...
describe("vaultTreeFlatten", () => {
    ...
    it("includes children when parent is expanded", () => {
        const items = [makeDoc("a", "A"), makeDoc("b", "B", "a")];
        const tree = vaultTreeBuild(items);
        const flat = vaultTreeFlatten(tree, new Set(["a"]));
    ...
    it("handles deep expansion", () => {
        ...
        const tree = vaultTreeBuild(items);
        const flat = vaultTreeFlatten(tree, new Set(["a", "b"]));
