...
import { vaultHtmlToMarkdown } from "@/modules/documents/vaultHtmlToMarkdown";
import { vaultEditorHtml } from "./vaultEditorHtml";
 ...
 */
export const VaultEditorView = React.memo<VaultEditorViewProps>(({ markdown, onChange }) => {
    ...
    const editorHtml = React.useMemo(() => vaultEditorHtml(initialHtml, colors), [initialHtml, colors]);
    ...
    React.useEffect(() => {
        ...
        const handler = (event: MessageEvent) => {
            if (event.data?.type === "content" && typeof event.data.html === "string") {
                const md = vaultHtmlToMarkdown(event.data.html);
                onChange(md);
            }
        };
                    ...
                    onMessage={(event) => {
                        try {
                            ...
                            if (data.type === "content" && typeof data.html === "string") {
                                const md = vaultHtmlToMarkdown(data.html);
                                onChange(md);
                            }
                        } catch {
                            // ignore parse errors
