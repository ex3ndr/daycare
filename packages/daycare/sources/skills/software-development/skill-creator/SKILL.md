...
Skills are modular, self-contained packages that extend Daycare's capabilities by providing
...
- `tools` (optional string array): tool names this skill documents. Keep it to a flat list of exact tool ids.
...
3. **Bundled resources** - As needed by Daycare (Unlimited because scripts can be executed without reading into context window)
...
- `tools` (optional): exact tool names documented by this skill. Use this when the skill is meant to show a hidden-by-default tool surface in prompt form.
