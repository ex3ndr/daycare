...
Stream state progressively - output one patch per array item instead of one giant blob:
  ...
  For scalars: {"op":"add","path":"/state/newItemText","value":""}
...
Example: {"type":"View","props":{},"repeat":{"statePath":"/items","key":"id"},"children":["item-row"]}
...
Example: on: { "press": { "action": "pushState", "params": { "statePath": "/items", "value": { "id": "$id", "title": { "$state": "/newItemText" }, "completed": false }, "clearStatePath": "/newItemText" } } }
...
For lists where users can add/remove items (carts, contacts, playlists, etc.), use pushState and removeState instead of hardcoding with setState.

IMPORTANT: State paths use RFC 6901 JSON Pointer syntax (e.g. "/items/0/title"). Do NOT use JavaScript-style dot notation (e.g. "/items.length" is WRONG). To generate unique IDs for new items, use "$id" instead of trying to read array length.
...
12. When building repeating content backed by a state array (e.g. posts, cart items, contacts), use the "repeat" field on a container element from the AVAILABLE COMPONENTS list. Example: { "type": "<ContainerComponent>", "props": { "gap": 8 }, "repeat": { "statePath": "/items", "key": "id" }, "children": ["item-row"] }. Inside repeated children, use { "$item": "field" } to read a field from the current item, and { "$index": true } for the current array index. For two-way binding to an item field use { "$bindItem": "completed" }. Do NOT hardcode individual elements for each array item.
