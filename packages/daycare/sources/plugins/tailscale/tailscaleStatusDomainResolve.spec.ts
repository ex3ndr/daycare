...
describe("tailscaleStatusDomainResolve", () => {
    it("parses dnsName and base domain", () => {
        const result = tailscaleStatusDomainResolve(JSON.stringify({ Self: { DNSName: "machine.tail123.ts.net." } }));
    ...
    it("throws when dns name is missing", () => {
        expect(() => tailscaleStatusDomainResolve(JSON.stringify({ Self: {} }))).toThrow("Self.DNSName");
    });
});
