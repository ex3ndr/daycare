...
describe("tailscaleTunnelCommandBuild", () => {
  it("builds create commands for public and local-network modes", () => {
    expect(
      tailscaleTunnelCommandBuild({ action: "create", mode: "public", proxyPort: 7777 })
    ).toEqual({
      command: "tailscale",
  ...
  it("builds destroy commands for public and local-network modes", () => {
    expect(
      tailscaleTunnelCommandBuild({ action: "destroy", mode: "public" })
    ).toEqual({
      command: "tailscale",
    ...
    expect(
      tailscaleTunnelCommandBuild({ action: "destroy", mode: "local-network" })
    ).toEqual({
      command: "tailscale",
