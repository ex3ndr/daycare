...
describe("dashboardProxyPathResolve", () => {
  it("strips /api prefix for engine routes", () => {
    expect(dashboardProxyPathResolve("/api/v1/engine/heartbeat/tasks", "")).toBe(
      "/v1/engine/heartbeat/tasks"
    );
  });

  it("preserves already-normalized engine routes", () => {
    expect(dashboardProxyPathResolve("/v1/engine/status", "?verbose=1")).toBe(
      "/v1/engine/status?verbose=1"
    );
  });
