...
describe("dashboard plugin onboarding", () => {
    it("stores host and port when auth is disabled", async () => {
        const prompt = {
            input: vi.fn().mockResolvedValueOnce("127.0.0.1").mockResolvedValueOnce("7331"),
    ...
    it("hashes generated password when auth is enabled", async () => {
        ...
        const auth = result?.settings?.basicAuth as { username: string; passwordHash: string } | undefined;
        if (!auth) {
