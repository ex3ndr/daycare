...
async function resolveWritePathSecure(permissions: SessionPermissions, target: string): Promise<string> {
    return sandboxCanWrite(permissions, target);
}

async function resolveReadPathSecure(permissions: SessionPermissions, target: string): Promise<string> {
