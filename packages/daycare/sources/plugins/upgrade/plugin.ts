...
export const plugin = definePlugin({
  ...
  create: (api) => {
    ...
    const buildSelfUpgradeTool = (): ToolDefinition<
      typeof selfUpgradeToolSchema,
      SelfUpgradeResult
    > => ({
      tool: {
        name: "self_upgrade",
      ...
      execute: async (args, toolContext, toolCall) => {
        ...
        api.logger.info(
          { previousVersion, versionToInstall },
          "self_upgrade: Starting upgrade"
        );
        ...
        if (previousVersion) {
          try {
            ...
            api.logger.warn(
              { error },
              "self_upgrade: Failed to persist upgrade pending marker"
            );
          }
        }
      ...
      postStart: async () => {
        ...
        if (pending.previousVersion) {
          ...
          try {
            ...
            api.logger.warn(
              { error },
              "error: Failed to send upgrade completion status message"
            );
          }
          return;
        }
        try {
          ...
          api.logger.warn(
            { error },
            "error: Failed to send restart completion status message"
          );
        }
      }
    };
