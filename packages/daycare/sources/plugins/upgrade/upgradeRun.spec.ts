...
describe("upgradeRun", () => {
    ...
    it("runs npm install then pm2 restart and reports status", async () => {
        ...
        expect(execFileMock).toHaveBeenNthCalledWith(2, "pm2", ["jlist"], { windowsHide: true }, expect.any(Function));
        expect(execFileMock).toHaveBeenNthCalledWith(
            3,
            "pm2",
            ["restart", "daycare"],
    ...
    it("reports npm install failures and aborts restart", async () => {
        ...
        ).rejects.toThrow("Upgrade failed while installing daycare-cli: permission denied");

        expect(execFileMock).toHaveBeenCalledTimes(1);
        expect(sendStatus).toHaveBeenCalledWith("Upgrade failed while installing daycare-cli: permission denied");
    });
    ...
    it("throws restart failures when pm2 snapshot does not indicate success", async () => {
        ...
        ).rejects.toThrow('Upgrade failed while restarting PM2 process "daycare": pm2 not found');
