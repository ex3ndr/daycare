...
const settingsSchema = z
    .object({
        allowedPhones: z
            .array(phoneSchema)
            .min(1)
            .transform((values) => Array.from(new Set(values.map((value) => String(value).replace(/\D/g, ""))))),
        printQRInTerminal: z.boolean().optional()
    })
    .passthrough();

type WhatsAppPluginConfig = Omit<WhatsAppConnectorOptions, "authStore" | "instanceId" | "fileStore" | "dataDir">;
...
async function promptAllowedPhones(api: PluginOnboardingApi): Promise<string[] | null> {
    for (;;) {
