...
export async function setDefaultProviderCommand(options: ProviderCommandOptions): Promise<void> {
    ...
    const configuredProviders = listProviders(settings).filter((provider) => provider.enabled !== false);
    const providers: ConnectedProvider[] = connected ?? configuredProviders.map((provider) => ({ id: provider.id }));
    ...
    if (definition.models && definition.models.length > 0) {
        ...
        const modelChoices = [
            ...
            ...definition.models.map((model) => ({
                ...
                description: model.size === "unknown" ? model.id : `${model.id} · ${model.size}`
            }))
        ];
