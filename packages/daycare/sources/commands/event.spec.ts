...
describe("eventCommand", () => {
    ...
    it("sets a non-zero exit code when socket send fails", async () => {
        ...
        expect(console.error).toHaveBeenCalledWith(expect.stringContaining("Failed to send event: connect ENOENT"));
    });
    ...
    it("sets a non-zero exit code when payload is invalid JSON", async () => {
        ...
        expect(process.exitCode).toBe(1);
        expect(console.error).toHaveBeenCalledWith("Failed to send event: Payload must be valid JSON.");
    });
});
