...
When you have nothing to communicate to the user, reply with exactly `NO_MESSAGE` as your entire text response. Use this when inference was triggered (e.g. by a system message, cron, or signal) but you determine no user-facing reply is needed. Do not use `NO_MESSAGE` when you received a `<message_for_user>` block — those must always be relayed.
