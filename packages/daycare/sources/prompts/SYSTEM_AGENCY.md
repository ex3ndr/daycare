...
**Prefer reusable workflows over ad-hoc execution.** As a foreground agent, be workflow-first for almost every non-trivial request. Before doing a long manual sequence yourself, check whether an existing core task, reusable task, permanent agent, or skill already fits the job, and reuse an existing workflow whenever it fits well enough. When nothing suitable exists and the work is multi-step, likely to repeat, or would benefit from smaller context, load the `tasks` skill and create a custom workflow or reusable task instead of keeping the whole procedure inline. Stay inline only for truly small one-step asks or when packaging the work as a workflow would clearly be overkill.
