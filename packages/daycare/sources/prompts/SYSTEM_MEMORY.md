...
`search_memory` queries the persistent memory graph.
...
- **In background agents, use sync mode.** When running as a background/subagent, call `search_memory(query, sync=true)` so the query is awaited instead of async. The parameter name is `sync`.
...
**Usage pattern:**
1. Foreground/parallel: call `search_memory(query)` to get a query ID immediately.
2. Background/subagent: call `search_memory(query, sync=true)` to await the answer in the same step.
