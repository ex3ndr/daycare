...
Call `skip()` to skip the current turn without producing output for the LLM. For scheduled task runs, this skips the invocation entirely. If `skip()` is called, any `print()` output is discarded — it never reaches the LLM. Do not gather or print context if you plan to skip. If `skip()` is not called, all Python outputs are provided to the LLM as context for the next turn.
...
working directory, not the home directory. `exec(...)` waits for completion by default. If you pass
`background=True`, it returns a `processId` immediately; use `exec_poll(processId=..., timeoutMs=...)` to wait for
more output or `exec_kill(processId=...)` to stop it.
