...
describe("tagExtractAllWithAttrs", () => {
    ...
    it("supports single-quoted attributes", () => {
        ...
        expect(tagExtractAllWithAttrs(text, "file")).toEqual([{ content: "/tmp/a.png", attrs: { mode: "photo" } }]);
    });

    it("returns attrs as empty object when absent", () => {
        expect(tagExtractAllWithAttrs("<file> a </file>", "file")).toEqual([{ content: "a", attrs: {} }]);
    });
    ...
    it("handles mixed content around tags", () => {
        ...
        expect(tagExtractAllWithAttrs(text, "file")).toEqual([{ content: "/tmp/a.mp4", attrs: { mode: "video" } }]);
    });
});
