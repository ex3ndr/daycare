...
export class FileStore {
    ...
    async saveBuffer(options: { name: string; mimeType: string; data: Buffer; source: string }): Promise<StoredFile> {
    ...
    async saveFromPath(options: { name: string; mimeType: string; source: string; path: string }): Promise<StoredFile> {
        await this.ensureDir();
        const id = createId();
        const filename = `${id}__${sanitizeFilename(options.name)}`;
        const filePath = path.join(this.resolvePath(), filename);
