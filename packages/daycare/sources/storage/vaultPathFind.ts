 ...
 */
export async function vaultPathFind(ctx: Context, path: string, repo: VaultPathFindRepo): Promise<string | null> {
    const normalized = path.trim();
    const remainder = vaultPathRemainderExtract(normalized);
    if (remainder === null) {
        return null;
    }
...
function vaultPathRemainderExtract(path: string): string | null {
