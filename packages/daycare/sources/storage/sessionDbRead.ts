 ...
 */
export async function sessionDbRead(config: Config, sessionId: string): Promise<SessionDbRecord | null> {
    const db = databaseOpenEnsured(config.dbPath);
    try {
        const row = db.prepare("SELECT * FROM sessions WHERE id = ? LIMIT 1").get(sessionId) as
            | DatabaseSessionRow
            | undefined;
        if (!row) {
            return null;
