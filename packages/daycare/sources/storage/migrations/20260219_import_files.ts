...
export const migration20260219ImportFiles: Migration = {
  ...
  up(db): void {
    ...
    for (const entry of entries) {
      ...
      db.prepare("UPDATE agents SET active_session_id = ? WHERE id = ?").run(
        activeSessionId,
        agentId
      );
    }
...
function legacyStateRead(basePath: string): LegacyState | null {
  ...
  try {
    ...
    return {
      inferenceSessionId:
        typeof parsed.inferenceSessionId === "string" ? parsed.inferenceSessionId : undefined,
