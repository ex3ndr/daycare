...
import { type VaultBodyRefsRepo, vaultBodyRefs } from "./vaultBodyRefs.js";

describe("vaultBodyRefs", () => {
    it("extracts and resolves wiki links", async () => {
        ...
        const refs = await vaultBodyRefs(body, ctx, repo);
    ...
    it("requires vault:// for wiki-link path targets", async () => {
        ...
        const refs = await vaultBodyRefs("[[memory/daily]] [[~/memory/daily]] [[vault://memory/daily]]", ctx, repo);
    ...
    it("still resolves legacy doc:// wiki-link targets from stored content", async () => {
        ...
        const refs = await vaultBodyRefs("[[doc://document/mission]]", ctx, repo);
    ...
    it("returns empty list when no links resolve", async () => {
        ...
        const refs = await vaultBodyRefs("No refs here [[ ]].", ctx, repo);
        expect(refs).toEqual([]);
    });
    ...
    it("does not leak regex state across calls after an error", async () => {
        ...
        await expect(vaultBodyRefs("[[doc1]] [[doc2]]", ctx, repo)).rejects.toThrow("boom");
        throwOnDoc2 = false;
        await expect(vaultBodyRefs("[[doc1]]", ctx, repo)).resolves.toEqual(["doc1"]);
    });
});
