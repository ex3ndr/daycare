...
describe("VaultsRepository", () => {
    it("supports hierarchical CRUD with path resolution and delete guard", async () => {
        ...
        try {
            const repo = new VaultsRepository(storage.db);
            ...
            const path = await vaultPathResolve(ctx, "events", repo);
            expect(path).toBe("vault://memory/daily/events");

            const byPath = await vaultPathFind(ctx, "vault://memory/daily/events", repo);
    ...
    it("round-trips path-safe slugs through resolve/find", async () => {
        ...
        try {
            ...
            const path = await vaultPathResolve(ctx, "user-doc", repo);
            expect(path).toBe("vault://memory/user_profile-1.2");
            expect(await vaultPathFind(ctx, String(path ?? ""), repo)).toBe("user-doc");
        } finally {
            storage.connection.close();
        }
    });
