...
import { type VaultPathResolveRepo, vaultPathResolve } from "./vaultPathResolve.js";

describe("vaultPathResolve", () => {
    it("builds a nested path from parent chain", async () => {
        ...
        const repo: VaultPathResolveRepo = {
            findById: async (_ctx, id) => docs.get(id) ?? null,
            findParentId: async (_ctx, id) => parents.get(id) ?? null
        };

        const resolved = await vaultPathResolve(ctx, "doc-events", repo);
    ...
    it("renders the stored root slug `document` as the public vault root", async () => {
        ...
        expect(await vaultPathResolve(ctx, "doc-mission", repo)).toBe("vault://vault/mission");
    });
    ...
    it("returns null when document is missing", async () => {
        ...
        expect(await vaultPathResolve(ctx, "missing", repo)).toBeNull();
    });
    ...
    it("throws on parent cycles", async () => {
        ...
        await expect(vaultPathResolve(ctx, "doc-a", repo)).rejects.toThrow("cycle");
    });
});
