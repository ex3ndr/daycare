...
describe("tagExtract", () => {
    ...
    it("handles tags with attributes", () => {
        ...
        expect(tagExtract('<response type="final" id="1">content</response>', "response")).toBe("content");
    });

    it("preserves inner tags unmodified", () => {
        expect(tagExtract("<response>has <inner> tags</response>", "response")).toBe("has <inner> tags");
    });
    ...
    it("uses first open and last close for multiple occurrences", () => {
        ...
        expect(tagExtract(text, "response")).toBe("outer <response>inner</response> middle");
    });
...
describe("tagStrip", () => {
    it("removes the tag block from text", () => {
        expect(tagStrip("prefix <response>content</response> suffix", "response")).toBe("prefix  suffix");
    });
    ...
    it("uses first open and last close", () => {
        expect(tagStrip("before <response>a</response> mid <response>b</response> after", "response")).toBe(
            "before  after"
        );
    });
