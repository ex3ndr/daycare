...
describe("sandboxPathContainerToHost", () => {
    ...
    it("rewrites container home path to host home", () => {
        const rewritten = sandboxPathContainerToHost({ hostHomeDir, targetPath: "/home" });
    ...
    it("rewrites nested container home paths", () => {
        const rewritten = sandboxPathContainerToHost({
            hostHomeDir,
            targetPath: "/home/desktop/project/file.ts"
        });
    ...
    it("keeps non-mapped container paths unchanged", () => {
        ...
        const rewritten = sandboxPathContainerToHost({ hostHomeDir, targetPath: outsidePath });
        expect(rewritten).toBeNull();
    });

    it("keeps relative paths unchanged", () => {
        const relativePath = "home/desktop/file.ts";
        const rewritten = sandboxPathContainerToHost({ hostHomeDir, targetPath: relativePath });
        expect(rewritten).toBeNull();
    });
    ...
    it("rewrites nested /shared/skills paths to host active skills root", () => {
        const rewritten = sandboxPathContainerToHost({
            hostHomeDir,
            targetPath: "/shared/skills/core--deploy/SKILL.md",
            hostSkillsActiveDir
        });
