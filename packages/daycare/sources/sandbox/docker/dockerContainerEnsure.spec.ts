...
describe("dockerContainerEnsure", () => {
    ...
    it("recreates container when /run tmpfs label is missing", async () => {
        const existing = {
            inspect: vi.fn().mockResolvedValue({
                State: { Running: true },
                Config: {
    ...
    it("passes Init: true in HostConfig when creating container", async () => {
        ...
        const createContainerMock = docker.createContainer as ReturnType<typeof vi.fn>;

        expect(createContainerMock).toHaveBeenCalledTimes(1);
        const createCall = createContainerMock.mock.calls.at(0)?.at(0);
