...
const baseUnrestrictedRuntimeConfig = {
    filesystem: {
        denyRead: [],
        allowWrite: ["."],
        denyWrite: []
...
let unrestrictedRuntimeInitialized = false;
...
export async function runInSandbox(
    ...
    const runtimeConfig: SandboxRuntimeConfigWithOptionalGlobalNetwork = {
        ...config,
        ...(enableWeakerNestedSandbox === undefined ? {} : { enableWeakerNestedSandbox })
    };
    if (runtimeConfig.network.allowedDomains === undefined) {
        return runInSandboxWithUnrestrictedNetwork(command, runtimeConfig, options);
    }
    return runInSandboxWithAllowedDomains(command, runtimeConfig as SandboxRuntimeConfig, options);
}

async function runInSandboxWithAllowedDomains(
    command: string,
...
async function runInSandboxWithUnrestrictedNetwork(
    command: string,
    runtimeConfig: SandboxRuntimeConfigWithOptionalGlobalNetwork,
    options: {
        cwd?: string;
        env?: NodeJS.ProcessEnv;
        home?: string;
    ...
    await sandboxRuntimeUnrestrictedInitialize();
    logger.debug("execute: Executing command with sandbox config (unrestricted network)");
    const baseEnv = options.env ?? process.env;
    const { env } = await sandboxHomeRedefine({
        env: baseEnv,
        home: options.home
    ...
    const wrappedCommand = await SandboxManager.wrapWithSandbox(
        ...
        "/bin/bash",
        {
            ...
            ...(runtimeConfig.enableWeakerNestedSandbox === true ? { enableWeakerNestedSandbox: true } : {})
        },
        options.signal
    );
...
async function sandboxRuntimeUnrestrictedInitialize(): Promise<void> {
    if (unrestrictedRuntimeInitialized) {
        return;
    }
    await SandboxManager.initialize(baseUnrestrictedRuntimeConfig);
    unrestrictedRuntimeInitialized = true;
}
