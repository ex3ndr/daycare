 ...
 */
export async function channelLoad(
  baseDir: string,
  name: string
...
export async function channelSave(baseDir: string, channel: Channel): Promise<void> {
  ...
  await atomicWrite(
    channelFilePathBuild(baseDir, channelName),
 ...
 */
export async function channelAppendMessage(
  baseDir: string,
  name: string,
  ...
  await fs.mkdir(directory, { recursive: true });
  await fs.appendFile(
    channelHistoryPathBuild(baseDir, channelName),
 ...
 */
export async function channelReadHistory(
  baseDir: string,
  name: string,
  limit = 50
