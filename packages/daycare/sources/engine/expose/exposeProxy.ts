...
export class ExposeProxy {
    ...
    updateRoute(domain: string, update: ExposeProxyUpdateInput): void {
        ...
        this.routes.set(normalizedDomain, {
            ...current,
            passwordHash: "passwordHash" in update ? (update.passwordHash ?? null) : current.passwordHash
        });
    }

    private async routeRequest(request: FastifyRequest, reply: FastifyReply): Promise<void> {
        ...
        if (route.passwordHash) {
            const authorized = await authorizationCheck(request.headers.authorization, route.passwordHash);
            if (!authorized) {
                reply.status(401).header("WWW-Authenticate", 'Basic realm="daycare"').send("Unauthorized");
                return;
            }
        }

        await this.proxyRequest(request, reply, route.target, route.passwordHash !== null);
    }
