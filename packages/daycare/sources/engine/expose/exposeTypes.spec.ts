...
describe("exposeTypes", () => {
  ...
  it("normalizes valid domains", () => {
    ...
    expect(() => exposeDomainNormalize("invalid_domain")).toThrow(
      "Invalid expose domain"
    );
  });
  ...
  it("checks mode capability support", () => {
    expect(
      exposeModeSupported("public", { public: true, localNetwork: false })
    ).toBe(true);
    expect(
      exposeModeSupported("local-network", { public: true, localNetwork: false })
    ).toBe(false);
  });
});
