 ...
 */
export function execGateOutputAppend(prompt: string, result: ExecGateCheckResult): string {
...
function formatGateOutput(stdout: string, stderr: string): string {
    const parts = [stdout, stderr].map((value) => value.trim()).filter((value) => value.length > 0);
    return parts.join("\n").trim();
}
