...
export async function appReviewPromptBuild(input: AppReviewPromptBuildInput): Promise<string> {
    ...
    const allowRules =
        input.rules.allow.length > 0 ? input.rules.allow.map((rule) => `- ${rule.text}`).join("\n") : "- (none)";
    const denyRules =
        input.rules.deny.length > 0 ? input.rules.deny.map((rule) => `- ${rule.text}`).join("\n") : "- (none)";
...
function toolsSerialize(tools: Array<{ name: string; description: string; parameters: unknown }>): string {
    if (tools.length === 0) {
        return "- (none)";
    }
    return tools
