    ...
    return {
        ...
        execute: async (toolCall, context) => {
            if (!allowedToolNames.has(toolCall.name)) {
                ...
                return toolExecutionResultOutcome(
                    appToolErrorBuild(toolCall, `Tool "${toolCall.name}" is not available in app sandbox.`)
                );
            }
            ...
            appLogger.info({ appId: input.appId, tool: toolCall.name, allowed: true }, "review: App tool call allowed");
            return input.toolResolver.execute(toolCall, context);
        }
    };
}
