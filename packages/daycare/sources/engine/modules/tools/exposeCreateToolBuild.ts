 ...
 */
export function exposeCreateToolBuild(exposes: Pick<Exposes, "create">): ToolDefinition {
    return {
        tool: {
            name: "expose_create",
            description: "Expose a local HTTP port or unix socket through a tunnel provider.",
            parameters: schema
        },
...
function targetResolve(payload: ExposeCreateArgs): ExposeTarget {
    ...
    const hasUnixSocket = typeof payload.unixSocket === "string" && payload.unixSocket.trim().length > 0;
