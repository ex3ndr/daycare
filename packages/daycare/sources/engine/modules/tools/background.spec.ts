...
describe("buildStartBackgroundAgentTool", () => {
    it("grants validated permissions before posting the first message", async () => {
        ...
        expect(grantPermission).toHaveBeenNthCalledWith(1, { agentId: "agent-123" }, { kind: "network" });
        expect(grantPermission).toHaveBeenNthCalledWith(2, { agentId: "agent-123" }, { kind: "read", path: "/tmp" });
    ...
    it("rejects permissions the creator does not have", async () => {
        ...
        await expect(tool.execute({ prompt: "Do work", permissions: ["@network"] }, context, toolCall)).rejects.toThrow(
