    ...
    return {
        ...
        execute: async (args, toolContext, toolCall) => {
            ...
            for (const requestedPermission of requestedPermissions) {
                const hasPermission = await permissionAccessAllows(targetPermissions, requestedPermission.access);
                if (!hasPermission) {
            ...
            const friendly = permissionsToRequest.map((entry) => `- ${describePermission(entry.access)}`).join("\n");
            const requesterLabel = agentDescriptorLabel(requestedDescriptor);
            const requesterKind = requestedDescriptor.type === "user" ? "foreground" : "background";
            const heading =
            ...
            const scopeLine = descriptor.type === "app" ? `Scope: ${permissionScopeLabel(requestedScope)}` : null;
            const text = [heading, friendly, `Reason: ${reason}`, scopeLine]
            ...
            try {
                decision = await permissionRequestRegistry.register(request.token, timeoutMinutes * 60_000);
            } catch (error) {
            ...
            if (decision.approved) {
                for (const permission of decision.permissions) {
                    ...
                    if (descriptor.type === "app" && resolvedScope === "always") {
                        await toolContext.agentSystem.grantAppPermission(descriptor.appId, permission.access, {
            ...
            const scopedResultText =
                descriptor.type === "app" ? `${resultText} Scope: ${permissionScopeLabel(resolvedScope)}.` : resultText;
...
export function buildPermissionGrantTool(): ToolDefinition<typeof grantSchema, PermissionToolResult> {
    return {
        tool: {
            ...
            description: "Grant a permission you already have to another agent (requires a justification).",
        ...
        execute: async (args, toolContext, toolCall) => {
            ...
            await toolContext.agentSystem.grantPermission({ agentId: payload.agentId }, access);
...
function permissionScopeLabel(scope: PermissionRequestScope): string {
    return scope === "always" ? "Always (all future runs for this app)" : "One time (this app run only)";
}
