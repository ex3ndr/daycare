...
describe("skillToolBuild", () => {
  ...
  it("runs sandbox skills in a subagent and returns subagent output", async () => {
    ...
    try {
      ...
      const postAndAwait = vi.fn(async () => ({ type: "message" as const, responseText: "Deployment complete." }));
      const tool = skillToolBuild();
      const context = contextBuild({
      ...
      const result = await tool.execute(
        { name: "deploy", prompt: "Deploy version 1.2.3" },
        context,
        toolCall
      );
  ...
  it("rejects sandbox skill permissions that exceed parent permissions", async () => {
    ...
    try {
      ...
      await expect(
        tool.execute({ name: "deploy", prompt: "Ship it" }, context, toolCall)
...
function contextBuild(input?: {
  ...
  const postAndAwait =
    input?.agentSystem?.postAndAwait ??
    (async () => ({ type: "message" as const, responseText: "ok" }));
...
function skillBuild(
  skillPath: string,
  overrides: Partial<AgentSkill> & Pick<AgentSkill, "name">
): AgentSkill {
