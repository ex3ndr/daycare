...
export class InferenceRegistry {
    ...
    register(pluginId: string, provider: InferenceProvider): void {
        this.logger.debug(
            `register: Registering inference provider pluginId=${pluginId} providerId=${provider.id} label=${provider.label}`
        );
    ...
    unregisterByPlugin(pluginId: string): void {
        ...
        this.logger.debug(
            `unregister: Inference providers unregistered by plugin pluginId=${pluginId} unregisteredCount=${count}`
        );
    }
