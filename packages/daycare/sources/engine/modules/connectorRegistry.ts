...
export class ConnectorRegistry {
    ...
    register(id: string, connector: Connector): ConnectorActionResult {
        ...
        this.logger.debug(
            `event: Connector added to registry connectorId=${id} totalConnectors=${this.connectors.size}`
        );
    ...
    async unregister(id: string, reason = "unload"): Promise<ConnectorActionResult> {
        ...
        this.logger.debug(
            `event: Connector removed from registry connectorId=${id} remainingConnectors=${this.connectors.size}`
        );
    ...
    private async updateCommandsForConnector(id: string, connector: Connector): Promise<void> {
        ...
        try {
            ...
            this.logger.warn({ connector: id, error }, "error: Connector updateCommands() failed");
        }
    }
}
