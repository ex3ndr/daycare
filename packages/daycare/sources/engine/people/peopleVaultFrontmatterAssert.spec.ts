...
import { peopleVaultFrontmatterAssert } from "./peopleVaultFrontmatterAssert.js";

describe("peopleVaultFrontmatterAssert", () => {
    it("skips non-people documents", async () => {
        const storage = await storageOpenTest();
        try {
            const ctx = contextForUser({ userId: "user-1" });
            await expect(
                peopleVaultFrontmatterAssert({
                    ctx,
    ...
    it("requires firstName frontmatter for documents inside vault://people", async () => {
        const storage = await storageOpenTest();
        try {
            const ctx = contextForUser({ userId: "user-1" });
    ...
    it("does not require person frontmatter for the vault://people root document", async () => {
        const storage = await storageOpenTest();
        try {
            const ctx = contextForUser({ userId: "user-1" });
            await expect(
