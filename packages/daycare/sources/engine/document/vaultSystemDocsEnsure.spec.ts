...
import { vaultSystemDocsEnsure } from "./vaultSystemDocsEnsure.js";

describe("vaultSystemDocsEnsure", () => {
    it("creates vault://system with bundled child prompt documents", async () => {
        const storage = await storageOpenTest();
        try {
            const ctx = contextForUser({ userId: "user-1" });

            const first = await vaultSystemDocsEnsure(ctx, storage);
            ...
            const memoryCompactor = await storage.documents.findBySlugAndParent(ctx, "compactor", memory?.id ?? "");
            ...
            expect(memoryCompactor?.body).toBe(await agentPromptBundledRead("MEMORY_COMPACTOR.md"));
    ...
    it("is idempotent and keeps existing child documents", async () => {
        ...
        try {
            ...
            const first = await vaultSystemDocsEnsure(ctx, storage, { soulBody: "initial soul\n" });
            ...
            const second = await vaultSystemDocsEnsure(ctx, storage, { soulBody: "replacement soul\n" });
