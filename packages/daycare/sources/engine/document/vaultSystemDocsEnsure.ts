 ...
 */
export async function vaultSystemDocsEnsure(
    ctx: Context,
    storage: Pick<Storage, "documents">,
    options?: { soulBody?: string }
): Promise<{ id: string; created: boolean }> {
    const root = await vaultEnsure(ctx, storage, {
    ...
    for (const doc of SYSTEM_DOCS) {
        ...
        const ensured = await vaultEnsure(ctx, storage, {
            slug: doc.slug,
            title: doc.title,
