...
import { vaultRootVaultEnsure } from "./vaultRootVaultEnsure.js";

describe("vaultRootVaultEnsure", () => {
    it("creates vault://vault when missing and is idempotent", async () => {
        const storage = await storageOpenTest();
        try {
            const ctx = contextForUser({ userId: "user-1" });

            const first = await vaultRootVaultEnsure(ctx, storage);
            ...
            const second = await vaultRootVaultEnsure(ctx, storage);
