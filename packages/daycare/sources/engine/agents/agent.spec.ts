...
describe("Agent", () => {
    ...
    it("allows step() during direct task execution and waits for the target agent", async () => {
        const dir = await mkdtemp(path.join(os.tmpdir(), "daycare-agent-task-step-"));
        try {
            const config = configResolve(
                {
                    engine: { dataDir: dir },
            ...
            const complete = vi.fn(async () => ({
                ...
                message: {
                    role: "assistant" as const,
                    content: [{ type: "text" as const, text: "stepped" }],
                    api: "openai-responses",
                    provider: "openai",
                    model: "gpt-4.1",
                    usage: {
            ...
            expect(userRecord.text).toContain("continue working");
        } finally {
            await rm(dir, { recursive: true, force: true });
        }
    });

    it("rejects step() outside task execution", async () => {
        const dir = await mkdtemp(path.join(os.tmpdir(), "daycare-agent-step-error-"));
        try {
            const config = configResolve(
                {
                    engine: { dataDir: dir },
            ...
            const result = await postAndAwait(
                agentSystem,
                { descriptor: { type: "cron", id: createId(), name: "step-test" } },
                {
                    type: "system_message",
                    text: "sync run",
                    code: "step('continue')",
            ...
            expect(result.responseError).toBe(true);
            expect(result.executionErrorText).toContain("step() is allowed only in tasks.");
        } finally {
            await rm(dir, { recursive: true, force: true });
        }
    });
    ...
    it("skips pre-turn compaction when the active model has a higher configured limit", async () => {
        const dir = await mkdtemp(path.join(os.tmpdir(), "daycare-agent-"));
        try {
            const config = configResolve(
                {
            ...
            const complete = vi.fn(async () => {
                ...
                return {
                    ...
                    message: {
                        role: "assistant",
                        content: [{ type: "text", text: "continued without compaction" }],
                        api: "openai-responses",
                        provider: "openai",
                        model: "gpt-4.1",
                        usage: {
            ...
            expect(result).toEqual({ type: "message", responseText: "continued without compaction" });
