...
Skills can now declare a `tools:` frontmatter list that shows hidden-by-default tool docs on demand.
...
flowchart TD
    ...
    B --> C[system prompt renders skill metadata with listed tool names]
    ...
    F --> G[agent can see documented tools in later run_python blocks]
```

## Notes

- `hiddenByDefault` hides a tool from default system prompt rendering only; it does not disable execution.
- Core skills now declare the specialized tool families they document: tasks plus channels/signals, fragments, friends, permanent agents, and psql.
