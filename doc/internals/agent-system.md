...
- load agent descriptors/state from SQLite + create inboxes
...
1. **load()** reads `agents` rows from SQLite, creates inboxes, queues restore (no execution)
...
flowchart TD
  ...
  Engine --> Upgrade[storageUpgrade]
  Upgrade --> SQLite[(daycare.db)]
